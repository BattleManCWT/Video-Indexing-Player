...
def compare_first_frames(query_path, db_videos, downsample_percent):
    """Compare the first frame of the query video with the first frame of each shot in the database videos."""
    # Read the first frame of the query video
    query_cap = cv2.VideoCapture(query_path)
    ret, query_frame = query_cap.read()
    query_cap.release()
    if not ret:
        print("Error: Can't read query video file.")
        return None

    query_frame_downsampled = downsample_frame(query_frame, downsample_percent)
    query_hist = calculate_histogram(query_frame_downsampled)

    closest_match = {'video': None, 'shot': None, 'score': float('inf')}

    for video_path, video_signature in db_videos.items():
        cap = cv2.VideoCapture(video_path)
        for shot_boundary in video_signature['shotBoundaries']:
            # Jump to the start frame of the shot
            cap.set(cv2.CAP_PROP_POS_FRAMES, shot_boundary[0])
            ret, frame = cap.read()
            if not ret:
                continue

            frame_downsampled = downsample_frame(frame, downsample_percent)
            frame_hist = calculate_histogram(frame_downsampled)

            # Calculate histogram difference
            hist_diff = cv2.compareHist(query_hist, frame_hist, cv2.HISTCMP_BHATTACHARYYA)

            if hist_diff < closest_match['score']:
                closest_match = {'video': video_path, 'shot': shot_boundary, 'score': hist_diff}
...
query_path = "Data/Queries/video10_1.mp4"
query_signature = detect_shot_boundaries(query_path, color_threshold,
                                         motion_threshold, downsample_percent)

with open('db_signatures.json') as f:
    db_videos = json.load(f)

# Call the compare_shot_signatures function
shot_similarities = compare_shot_signatures(query_signature, db_videos)
...
# Call the compare_first_frames function
first_frame_match = compare_first_frames(query_path, db_videos, downsample_percent)
if first_frame_match['video'] is not None:
    match_shot_time = [frame_to_time(first_frame_match['shot'][0]), frame_to_time(first_frame_match['shot'][1])]
    print(f"Closest first frame match is in video: {first_frame_match['video']}, shot: {match_shot_time} with a score of {first_frame_match['score']}")
else:
    print("No close match found for the first frame.")